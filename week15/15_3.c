...
void sort(ST x[], int n, char key[][10], int m) {
  ...
  for (i = 0; i < n - 1; i++)
    for (j = 0; j < n - i - 1; j++) {
      ...
      if (con1 > con2 || con1 == con2 && strcmp(x[j].name, x[j + 1].name) > 0) { // error 4
        v = x[j];
        x[j] = x[j + 1];
        x[j + 1] = v;
      }
...
int main(void) {
  ...
  char sortkey[3][10] = {"bachelor", "master", "doctor"}; // error 2
  int i;
  sort(s, 5, sortkey, 3);
  for (i = 0; i < 5; i++)
    printf("\n%s    %s", s[i].name, s[i].degree); // error 3
  return 0;
}
